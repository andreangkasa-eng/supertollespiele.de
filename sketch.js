...
function setup() {
  ...
  btnNextLevel = createButton('STARTE LEVEL 2'); styleButton(btnNextLevel, height/2 + 20, '#00BFFF');
  btnNextLevel.mousePressed(() => { currentLevel = 2; initLevel(2); gameState = "START"; updateUIState(); });

  btnFS = createButton('FULLSCREEN'); styleButton(btnFS, height/2 + 80, '#888');
  btnFS.mousePressed(toggleFS);

  btnRetry = createButton('NOCHMAL VERSUCHEN'); styleButton(btnRetry, height/2 + 20, '#FF4500');
  btnRetry.mousePressed(() => { if(currentLevel === 2 && gameState === "WIN") currentLevel = 1; initLevel(currentLevel); gameState = "START"; updateUIState(); });
...
function styleButton(btn, yPos, col) {
  ...
  btn.style('border-radius', '12px'); btn.style('border', '3px solid #000'); btn.style('z-index', '1000'); 
  btn.hide();
}
...
function drawSpaceBackground() {
  background(5, 5, 20); noStroke(); fill(60, 0, 120, 30); ellipse(width/2, height/2, width*1.2, height*0.8);
...
function updateGame() {
  ...
  if (currentLevel === 1) {
    ...
    for (let p of platforms) {
      if (player.x + player.w*0.6 > p.x && player.x + player.w*0.4 < p.x + p.w && player.y + player.h > p.y && player.y + player.h < p.y + p.h + player.velocity) {
    ...
    if (frameCount % 50 === 0) bullets.push({ x: player.x + width, y: ufoY + 40, speed: 7, offset: random(-150, 150) });
...
function die() {
  lives--; if (lives <= 0) { gameState = "GAMEOVER"; updateUIState(); }
  else { player.x = 100; player.y = (currentLevel === 1) ? height - 170 : height/2; player.velocity = 0; scrollX = 0; timer = 40; }
}
...
function drawMenu() {
  fill(0, 180); rect(0, 0, width, height); textAlign(CENTER, CENTER); fill(200); textSize(16); text("Noa Productions präsentiert:", width/2, height/2 - 100);
  fill(255, 204, 0); textSize(40); textStyle(BOLD); text("LEVEL " + currentLevel + (currentLevel === 1 ? ": DIE WIESE" : ": WELTRAUM"), width/2, height/2 - 55);
}
